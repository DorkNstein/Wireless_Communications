% Adds a burst noise to the given bit data matrix
...
% Generating the burst noise
%b_noise = zeros(size(bits));
...
%Choosing a random length for burst in the min and max range
        length(b_prob==1) = randi(burst_max - (burst_min -1),size(b_prob(b_prob==1),1),1 ) ...
...
for i=1:size(b_prob,1)
    ...
    if(b_prob(i) == 1)
        ...
        %Depending on the definition of burst
        
          % If all bits are to be flipped in that length
           % error = ones(length-1,1);
